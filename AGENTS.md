...
- Entities (`Bullet`, `Asteroid`, `Ship`, `Particle`) follow the same pattern: `dead` flag, `update(dt)`, `draw()`, then arrays are filtered each frame. New entities should match this.
- Screen edges are toroidal — positions go through `wrap(v, max)`, including bullets.
...
README mentions power-ups and a "estrella fugaz" asteroid type; neither is implemented in `game.js`. Trust the code.
